...
class Cli {
	...
	// method to find a vehicle to tow (accepts the Truck object)
	findVehicleToTow(truck: Truck): void {
		inquirer
			...
			.then((answers) => {
                // get the vehicle to Tow object
				const vehicleToTow = answers.vehicleToTow

                // ensure a vehicle is selected before checking if it can be towed
				if (vehicleToTow) {
					// don't allow the selected Truck to tow itself. Alert the user of the mistake.
					if (truck.vin === vehicleToTow.vin) {
						console.log("The truck cannot tow itself.")
						// check if the vehicle can be towed by the truck
					} else {
		...
		inquirer
			...
			.then((answers) => {
				...
				if (answers.action === "Print details") {
					...
					// find the selected vehicle and check that it is a truck. If yes, then tow, if not, then return message.
					for (let i = 0; i < this.vehicles.length; i++) {
						if (this.vehicles[i].vin === this.selectedVehicleVin) {
							if (this.vehicles[i] instanceof Truck) {
								...
								console.log(
									"This vehicle cannot tow. Only Trucks can tow other vehicles"
								)
							}
						}
					}
					...
					for (let i = 0; i < this.vehicles.length; i++) {
						if (this.vehicles[i].vin === this.selectedVehicleVin) {
							if (this.vehicles[i] instanceof Motorbike) {
								...
								console.log(
									"This vehicle cannot wheelie. Only Motorbikes can perform a wheelie"
								)
							}
						}
					}
